    ...
    <Box sx={{ flexGrow: 1, position: 'fixed', left:0, right:0, top:0, zIndex:1 }}>
      ...
      <div id="AppBar-test"> 
        <Toolbar>
          ...
          <Search component='form' id='search'>
            <SearchIconWrapper>
              <SearchIcon />
            </SearchIconWrapper>
            <StyledInputBase
    ...
    <Box display="flex" justifyContent="center" alignItems="center" sx={{ marginTop:'64px', width:'100%'}}>
      <Grid container width='700px' marginTop={12}>
        ...
        <Grid item xs={12} marginTop={5} display="flex" flexDirection='row' alignItems="center" justifyContent={'space-between'}>
          ...
          <Button id='favorbutton' startIcon={LikeState[0]?<ThumbUpAltIcon/>:<ThumbUpOffAltIcon/>} onClick={handleLike}>{`${LikeState[1]}`}</Button>
        </Grid>
        {/* avatar and subcrib */}
        <Grid item xs={12} marginTop={3} display="flex" flexDirection='row' alignItems="center" justifyContent={'space-between'}>
          ...
          <Button id='subcribButton' variant="outlined" onClick={handleSubcrib}>{SubcribState? 'unsubscrib':`subcrib`}</Button>
        </Grid>
        {/* tags */}
        <Grid item xs={12} marginTop={5}>
          ...
          <Box display={'flex'} flexWrap={'wrap'}>
            {tags.map((info, key) => (
              <Chip id='button' label={`#${info}`} clickable key={key} sx={{ margin:'10px'}}/>
            ))}
          </Box>
        </Grid>
        {/* ingredients */}
      ...
      <ButtonGroup sx={{position:'fixed', right:'1em', bottom:'1em'}} orientation="vertical">
        <Button id='favorbutton' startIcon={LikeState[0]?<ThumbUpAltIcon/>:<ThumbUpOffAltIcon/>} onClick={handleLike}/>
        <Button id='favorbutton' startIcon={markState?<FavoriteIcon/>:<FavoriteBorderIcon/>} onClick={handleMark}/>
      </ButtonGroup>
    </Box>
  );
}
